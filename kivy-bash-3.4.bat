...
set BIN=""
if EXIST %~dp0MinGW\msysgit\bin\nul set BIN=%~dp0MinGW\msysgit\bin\
...
start %COMSPEC% /c %BIN%rxvt -backspacekey  -sl 2500 -fg %FGCOLOR% -bg %BGCOLOR% -sr -fn Courier-12 -tn msys -geometry 80x25 -e MinGW/msys/bin/bash --rcfile kivybash-3.4.rc -i
exit

:startbash
call %COMSPEC% /c %BIN%bash --rcfile kivybash-3.4.rc -i
